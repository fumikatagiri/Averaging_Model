...
############# fit averaging model
### input file names
i.files = c('Pto_PtoAvrRpt2_2dpi_all.txt',
            'Pto_PtoAvrRpm1_2dpi_all.txt',
            'mock_flg22_2dpi_all.txt',
            'mock_elf18_2dpi_all.txt',
            'Alternaria_3dpi_all.txt')
...
for (file.numb in 1:length(i.files)) {
  ...
  if (file.numb == 5) {
    input.dat = input.dat[input.dat$genotype != 'pad3' & input.dat$genotype != 'pmr4',]
    colnames(input.dat)[colnames(input.dat) == 'disease'] = 'value'
    input.dat = input.dat[input.dat$value != '',]
  }
  ...
  outp.n = paste(input.d.n, var.kept, 'Ave.v4.output.txt', sep='.')
  boxp.n = paste(input.d.n, var.kept, 'Ave.v4.boxplot.pdf', sep='.')
  diag.n = paste(input.d.n, var.kept, 'Ave.v4.diagnostic.pdf', sep='.')
  plot.n = paste(input.d.n, var.kept, 'Ave.v4.reconstitution.pdf', sep='.')
...
### import output file
i.files = c('Pto_PtoAvrRpt2_2dpi_all.txt.ABCD.Ave.v4.output',
            'Pto_PtoAvrRpm1_2dpi_all.txt.ABCD.Ave.v4.output',
            'mock_flg22_2dpi_all.txt.ABCD.Ave.v4.output',
            'mock_elf18_2dpi_all.txt.BCD.Ave.v4.output',
            'Alternaria_3dpi_all.txt.AB.Ave.v4.output')
set.n = c('AvrRpt2-ETI','AvrRpm1-ETI', 'flg22-PTI', 'elf18-PTI', 'Alternaria')
names(i.files) = set.n

i.files = i.files[1:4]  # forget aboutt Alternaria for this
set.n = names(i.files) 
...
for (d.set in set.n) {
  lm.out.file = paste0('bug.data/', i.files[d.set], '.txt')
...
pdf('reanalyzed.w.average.model.wremove.v4.pdf', width = 12, height = 8.5)
